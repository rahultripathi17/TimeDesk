...
export default function HrReportsPage() {
  ...
  const fetchDepartments = async () => {
    try {
        // Reuse Admin API (secured by middleware/Role)
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                                ...
                                <table className="w-full caption-bottom text-sm text-left">
                                    <thead className="[&_tr]:border-b bg-slate-50">
                                        <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                            ...
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground text-center">Absent Days</th>
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground text-center">Shortfall (Hrs)</th>
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground text-center">Missed Checkout</th>
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground text-right">Violation Score</th>
                                        </tr>
                                    </thead>
                                    <tbody className="[&_tr:last-child]:border-0">
                                        {loading ? (
                                            ...
                                            reportData.map((item, index) => (
                                                <tr key={item.user.id} className="border-b transition-colors hover:bg-slate-50">
                                                    <td className="p-4 align-middle font-medium text-slate-500">#{(page - 1) * LIMIT + index + 1}</td>
                                                    <td className="p-4 align-middle">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="flex flex-col">
                                                                <span className="font-medium text-slate-900">{item.user.full_name}</span>
                                                                <span className="text-xs text-slate-500">{item.user.designation || 'Employee'}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="p-4 align-middle">
                                                        <span className="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600 ring-1 ring-inset ring-slate-500/10">
                                                            {item.user.department || 'Unassigned'}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="p-4 align-middle text-center">
                                                        {item.shortfallMinutes > 0 ? (
                                                            <span className="font-bold text-orange-600">{(item.shortfallMinutes / 60).toFixed(1)}h</span>
                                                        ) : (
                                                            <span className="text-slate-400">-</span>
                                                        )}
                                                    </td>
                                                    <td className="p-4 align-middle text-center">
                                                        {item.missedCheckout > 0 ? (
                                                            <span className="font-bold text-slate-700">{item.missedCheckout}</span>
                                                        ) : (
                                                            <span className="text-slate-400">-</span>
                                                        )}
                                                    </td>
                                                    <td className="p-4 align-middle text-right">
                                                        <span className="font-mono font-bold text-red-700">{item.totalScore.toFixed(1)}</span>
                                                    </td>
                                                </tr>
                                            ))
                                        )}
            ...
            <TabsContent value="performers">
                <Card className="border-t-4 border-t-emerald-500 shadow-sm">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Employees with the highest attendance, punctuality, and extra hours for {format(new Date(parseInt(selectedYear), parseInt(selectedMonth) - 1), 'MMMM yyyy')}.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="rounded-md border">
                                ...
                                <table className="w-full caption-bottom text-sm text-left">
                                    <thead className="[&_tr]:border-b bg-slate-50">
                                        <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                            ...
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground">Employee</th>
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground">Department</th>
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground text-center">Present Days</th>
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground text-center">Overtime (Hrs)</th>
                                            <th className="h-12 px-4 align-middle font-medium text-muted-foreground text-center">Leaves Taken</th>
                                        </tr>
                                    </thead>
                                    <tbody className="[&_tr:last-child]:border-0">
                                        {loading ? (
                                            ...
                                            reportData.map((item, index) => (
                                                <tr key={item.user.id} className="border-b transition-colors hover:bg-slate-50">
                                                    <td className="p-4 align-middle font-medium text-slate-500">
                                                        {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `#${(page - 1) * LIMIT + index + 1}`}
                                                    </td>
                                                    <td className="p-4 align-middle">
                                                        <div className="flex items-center gap-3">
                                                            <Avatar className="h-9 w-9 border">
                                                    ...
                                                    <td className="p-4 align-middle text-center">
                                                        {item.overtimeMinutes > 0 ? (
                                                            <span className="font-bold text-blue-600">{(item.overtimeMinutes / 60).toFixed(1)}h</span>
                                                        ) : (
                                                            <span className="text-slate-400">-</span>
                                                        )}
                                                    </td>
                                                    <td className="p-4 align-middle text-center">
                                                        <span className={item.leaves === 0 ? "text-emerald-600 font-bold" : "text-slate-600"}>
                                                            {item.leaves}
                                                        </span>
                                                    </td>
                                                </tr>
            ...
            <TabsContent value="downloads">
                <Card className="border-t-4 border-t-indigo-500 shadow-sm">
                    <CardHeader>
                         <CardTitle className="text-xl font-bold text-slate-800">Muster Roll & Exports</CardTitle>
                         <CardDescription>Download detailed attendance sheets and reports</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <div className="grid gap-6 md:grid-cols-2">
                            {/* Card 1: Monthly Muster Roll */}
                            <div className="rounded-xl border bg-slate-50/50 p-6 flex flex-col gap-4">
                                <div className="flex items-start gap-4">
                                    <div className="rounded-lg bg-green-100 p-3">
                                        <FileSpreadsheet className="h-6 w-6 text-green-700" />
                                    </div>
                                    <div className="space-y-1">
                                        <h3 className="font-bold text-slate-900">Monthly Muster Roll</h3>
                                        <p className="text-sm text-slate-500">
                                            Detailed day-wise attendance matrix for all employees in a department. 
                                            Includes Present, Late, Leave, and Absent status with color coding.
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    >
                                        {isDownloading ? (
                                            ...
                                            <>
                                                ...
                                                Download {selectedDept !== 'all' ? selectedDept : ''} Excel
                                            </>
                                        )}
                                    </Button>
                                    {selectedDept === 'all' && (
                            ...
                            {/* Card 2: User Summary (Future) */}
                            <div className="rounded-xl border bg-slate-50/50 p-6 flex flex-col gap-4 opacity-50 cursor-not-allowed">
                                <div className="flex items-start gap-4">
                                    <div className="rounded-lg bg-blue-100 p-3">
                                        <FileUser className="h-6 w-6 text-blue-700" />
                                    </div>
                                    <div className="space-y-1">
                                        <h3 className="font-bold text-slate-900">Employee Breakdown</h3>
                                        <p className="text-sm text-slate-500">
                                            Individual employee report card with detailed punch-in/out logs and deviation analysis.
                                        </p>
                                    </div>
                                </div>
