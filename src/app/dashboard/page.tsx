...
export default function DashboardPage() {
  ...
  const [role, setRole] = useState<"employee" | "manager" | "hr" | "admin">(
    "employee"
  );
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      if (deviation < 0)
        msg += `Shortfall: ${Math.abs(Math.floor(deviation / 60))}h ${Math.abs(
          deviation % 60
        )}m.`;
    ...
    const formatWorkingDays = (days: number[]) => {
      ...
      if (isRange && sorted.length > 2) {
        return `${dayNames[sorted[0]]} - ${
          dayNames[sorted[sorted.length - 1]]
        }`;
      }
          ...
          <section className="grid gap-4 md:grid-cols-2 mb-6">
            <Card className="md:col-span-1 transition-all hover:shadow-md">
              ...
              <CardContent className="space-y-4">
                ...
                {status && (
                  <div
                    className={`flex items-center justify-between rounded-xl border px-4 py-3 text-xs ${currentStatus.bgColor} ${currentStatus.borderColor}`}
                  >
                    <div>
                      ...
                      <p
                        className={`mt-1 flex items-center gap-1.5 text-sm font-semibold ${currentStatus.color}`}
                      >
                    ...
                    <div className="text-right">
                      {checkInTime && (
                        <p className="text-[11px] text-slate-500">
                          In: {format(new Date(checkInTime), "h:mm a")}
                        </p>
                      )}
                      {checkOutTime && (
                        <p className="text-[11px] text-slate-500">
                          Out: {format(new Date(checkOutTime), "h:mm a")}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {!checkOutTime &&
                      ...
                      (() => {
                        ...
                        // UPDATE: User requested fixed 2 hours enablement for everyone
                        const minDuration = 2; // Hardcoded to 2 hours as per requirement

                        if (diffHours < minDuration) {
                          // Calculate remaining time
                          const remainingMs =
                            minDuration * 60 * 60 * 1000 - diffMs;
